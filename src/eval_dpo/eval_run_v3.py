...
def build_prompt(ctx: str, query: str) -> str:
    """
    v3: 强约束 JSON 输出（与 SFT v3 数据/评测脚本对齐）
    - 只允许输出一个 JSON 对象
    - 字段固定：refusal / answer / used_citations
    - 拒答时：answer 必须为 ""；used_citations 必须为 []
    """
    return f"""你将基于给定资料回答问题。

硬规则（必须遵守）：
1) 只输出一个 JSON 对象（不要输出任何其它文字、不要使用 Markdown 代码块、不要解释）。
2) JSON 字段固定为：
   - "refusal": true/false
   - "answer": string（拒答时必须为空字符串 ""）
   - "used_citations": string 数组（拒答时必须为空数组 []）
3) 若无法从【资料】中直接得到答案，或资料不足以支撑结论：必须拒答。
4) 若可以回答：answer 必须简洁；used_citations 必须列出支撑答案的 chunk_id（只能从【资料】中的方括号 chunk_id 选择）。
...
{{
  "refusal": true/false,
  "answer": "...",
  "used_citations": ["chunk_id1", "chunk_id2"]
}}
"""
...
def parse_model_output(raw_text: str) -> Tuple[bool, str, List[str], Dict[str, Any]]:
    """
    v3: 解析模型输出为 (refusal, answer, used_citations, meta)

    兼容策略：
    - 优先读取 used_citations
    - 若不存在，则兼容旧字段 citations
    - 若解析失败（非 JSON / JSON 不完整），进入 fallback：默认视为拒答（治理优先，防止胡答）
    """
    ...
    if isinstance(obj, dict):
        ...
        # 兼容旧字段：citations
        raw_cites = obj.get("used_citations")
        if raw_cites is None:
            raw_cites = obj.get("citations")
            if raw_cites is not None:
                meta["compat_citations_field"] = "citations"

        used_citations = normalize_citations(raw_cites)

        if not isinstance(answer, str):
            answer = "" if answer is None else str(answer)

        # 若 refusal 字段缺失：根据 answer 是否为空进行推断
        if refusal is None:
            refusal = (answer.strip() == "")
            meta["refusal_inferred"] = True

        # v3 严格语义：拒答 -> answer="" & used_citations=[]
        if bool(refusal):
            answer = ""
            used_citations = []

        return bool(refusal), answer.strip(), used_citations, meta

    # fallback：格式崩溃时默认拒答（避免自由生成污染评测/线上）
    meta["fallback"] = True
    return True, "", [], meta
...
def run_one(
    ...
    *,
    ...
    # v3: 只在“非拒答”且模型未给 citations 时才允许 autofill（避免拒答样本被硬塞引用）
    if (not final_refusal) and controls.autofill_citations and not used_citations and retrieved_chunk_ids:
        used_citations = retrieved_chunk_ids[: max(1, controls.autofill_topk)]
        parse_meta["citations_autofilled"] = True

    # v3: 严格语义——最终拒答必须 answer="" 且 used_citations=[]
    if final_refusal:
        answer = ""
        used_citations = []
