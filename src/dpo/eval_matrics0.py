...
"""
eval_matrics.py (v4)
- SoftCite: 证据文本相似度容忍（字符ngram cosine，适配中文/公式）
- AnswerSim/AnswerAcc: 轻量文本相似度（可留作参考）
- DeepSeek Judge: 语义正确率（Evidence-supported correctness）
  * Judge 只看 Question + Gold Evidence + Model Answer
  * 输出 JudgeScore(0~1) 与 JudgeAcc@tau
  * 带磁盘缓存，避免重复花钱
  * 支持并发（线程池）
- Refusal: RefusalRate(pred) + RefusalF1
  * should_refuse：gold_citations为空 或 top1_score < refusal_threshold（可选）

用法示例：
python src/dpo/eval_matrics.py \
  --test data/dpo/qa_raw_test.jsonl \
  --runs_dir runs \
  --chunks data/processed/chunks/chunks.jsonl \
  --settings sft \
  --out_csv tables/sft_v4.csv \
  --out_md  tables/sft_v4.md \
  --use_deepseek_judge \
...
def clean_answer_for_judge(ans: str) -> str:
    ans = (ans or "").strip()
...
def judge_one(
    ...
    if obj is None:
        ...
        score, reason = 0.0, f"judge_parse_fail: {content[:120]}"
    else:
...
def main():
    ...
    # outputs
    ap.add_argument("--out_csv", default="tables/metrics_v4.csv")
    ap.add_argument("--out_md", default="tables/metrics_v4.md")
    ...
    for setting in settings:
        ...
        for r in runs:
            ...
            parse_ok = bool(parsed.get("parse_ok", True))  # 你 runs 里一般 parse_ok=true 或缺省
            pred_refuse = bool(parsed.get("refusal")) if parse_ok else False
            pred_ans = str(parsed.get("answer") or "") if parse_ok else ""
            ...
            if not should_refuse:
                sim = cosine_sim_charngram(pred_ans, gold_ans)
                ans_sim_sum += sim
            ...
            # judge tasks（只在不该拒答且有 evidence 时）
            if args.use_deepseek_judge and (not should_refuse) and gold_evidence:
                # judge_tasks.append((qid, question, gold_evidence, clean_answer_for_judge(pred_ans)))
                judge_tasks.append((qid, question, gold_evidence, pred_ans))

        # run judge in parallel
        if args.use_deepseek_judge:
            if not args.deepseek_api_key:
                raise RuntimeError("你启用了 --use_deepseek_judge 但没有提供 --deepseek_api_key 或环境变量 DEEPSEEK_API_KEY")
        ...
        refusal_f1 = 0.0 if (refusal_prec + refusal_rec) == 0 else (2 * refusal_prec * refusal_rec / (refusal_prec + refusal_rec))
